...
def train(
  ...
  # print(valid_restore_cfg)
  # from t5x.backward_compatibility import V1_ASSIGNMENT_MAP, uio1_transform_fn
  # from dataclasses import replace
  # valid_restore_cfg = replace(
  #   valid_restore_cfg, assignment_map=V1_ASSIGNMENT_MAP, state_transformation_fns=[uio1_transform_fn])
  ...
  # This controls whether to log weights and the gradient norm
  if weight_metrics == "all":
    weight_computer = trainer_lib.WeightMetricsComputer()
    log_weights = False
  elif weight_metrics == "norm-and-weights":
    log_weights = True
    weight_computer = trainer_lib.GradNormComputer()
  elif weight_metrics == "norm":
    log_weights = False
    weight_computer = trainer_lib.GradNormComputer()
  else:
    if weight_metrics and weight_metrics != "none":
      raise ValueError(f"Unknown weight metrics {weight_metrics}")
    log_weights = False
  ...
  trainer = trainer_cls(
      ...
      weight_metrics_computer=weight_computer
  )
  del train_state
  ...
  if infer_eval_dataset_cfg is not None:
    # This is a hack to make sure the logger is used even if no metric is specified
    ...
    task = seqio.get_mixture_or_task(infer_eval_dataset_cfg.mixture_or_task_name)
    if isinstance(task, seqio.Mixture):
      tasks = task.tasks
    else:
      tasks = [task]
    for task in tasks:
      if not task._predict_with_aux_metric_fns:
        logging.info(f"Hacking task {task.name} to use null metrics")
        task._predict_with_aux_metric_fns = [null_metric]
        task._postprocess_fn = return_example
        task._score_metric_fns = []
        task._predict_metric_fns = []
  ...
  def _run_inference_eval():
    ...
    all_metrics, _, _ = evaluator.evaluate(trainer.train_state, train_state_axes)
  ...
  if use_gda:
    ...
    dummy_batch = jax.tree_map(lambda x: np.ones([1 if z is None else z for z in x.shape], x.dtype),
                               train_iter.element_spec)
