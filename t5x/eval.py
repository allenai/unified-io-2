...
from t5x.examples.unified_io.evaluator import UnifiedIOEvaluator
...
class InferenceEvaluator:
  ...
  def __init__(
    ...
    self._seqio_evaluator = inference_evaluator_cls(
        ...
        target_field_name="targets/text/tokens",
        **kwargs
    )
  ...
  def evaluate(
    ...
    all_metrics = self._seqio_evaluator.evaluate(
...
def evaluate(
  ...
  if log_only:
    logging.info("Log only model, inserting dummy metrics")
    ...
    task = seqio.get_mixture_or_task(dataset_cfg.mixture_or_task_name)
  ...
  if not evaluator.eval_tasks:
    task = seqio.get_mixture_or_task(dataset_cfg.mixture_or_task_name)
    if dataset_cfg.split not in task.splits:
      raise ValueError(f"'{dataset_cfg.mixture_or_task_name}' does not have "
                       f"split {dataset_cfg.split} (existing splits are {task.splits}).")
    raise ValueError(
  ...
  # This is a bit of a hack to get both the shape and dtype from `evaluator`, t5x
  # originally only gets shapes (why?) but that require the model to remember a
  # set of default dtype for it's inputs which amounts to a lot of book keeping since
  # UnifiedIO models can have a lot of inputs
  ds: tf.data.Dataset = next(iter(evaluator._seqio_evaluator._cached_model_datasets.values()))
  spec = {k: v for k, v in ds.element_spec[1].items() if v.dtype != tf.string}
  input_types = {k: getattr(jnp, v.dtype.name) for k, v in spec.items()}
  input_shapes = {k: (batch_size,) + v.shape for k, v in spec.items()}
  ...
  for train_state in train_state_initializer.from_checkpoints(
    ...
    if isinstance(all_metrics, tuple):
