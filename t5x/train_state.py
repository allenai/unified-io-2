...
class FlaxOptimTrainState(flax.struct.PyTreeNode):
  ...
  # Mode for the optimization -- GAN or normal training.
...
class GanOptimTrainState(FlaxOptimTrainState):
  """Simple train state for holding parameters, step, optimizer state for GAN training."""
  ...
  def apply_gradient(self,
    ...
    hparams = self._optimizer.optimizer_def.hyper_params
    new_optimizer = self._optimizer.apply_gradient(
        grads, 
        hyper_params=[
          hparams[0].replace(learning_rate=jnp.where(self.step % 2, 0.0, learning_rate)),
          hparams[1].replace(learning_rate=jnp.where(self.step % 2, learning_rate, 0.0))])
