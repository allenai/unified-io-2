...
import clu.metrics
import clu.values
from clu.data.dataset_iterator import ElementSpec
...
from t5x.examples.unified_io.modality_processing import get_target_modalities, get_input_modalities
from t5x.examples.unified_io.packing import PackingStrategy
from t5x.metrics import Time, AveragePerStep
...
class GradNormComputer(object):
  """Log grad-norm while being much more light weight then `WeightMetricsComputer`"""
  def compute_metrics(
...
class MetricsManager(object):
  ...
  def write_metrics_summary(self, metrics: MetricMapType, step: int,
    ...
    def _summarize_and_write():
      ...
      with self._writer_lock:
        metric_writers.write_values(self._writer, int(step), {k:v for k, v in summary.items() if 'image' not in k})
  
      if self.log_to_wandb:
        wandb_dict = {}
        for k, v in summary.items():
          if "image" in k and len(v.value.shape) == 3 and v.value.shape[-1] == 3:
            # Heuristic to detect an image sumary, TODO is there a better way?
            v = v.value
            if isinstance(v, jnp.ndarray):
              v = np.array(v)
            wandb_dict['image' + '/' + k] = wandb.Image(v, caption=k)
          else:
            if isinstance(v, clu.values.Scalar):
              v = v.value
              if isinstance(v, jnp.ndarray):
                v = np.array(v)
              key = self._name + "/" + k
              # Limit keys to a depth of 2 so we don't have too many wandb panes
              parts = key.split("/")
              if len(parts) > 2:
                key = parts[0] + "/" + parts[1] + "/" + "--".join(parts[2:])
              if isinstance(self.log_to_wandb, dict):
                if key in self.log_to_wandb:
                  key = self.log_to_wandb[key]
                  if key is None:
                    continue
              wandb_dict[key] = v
        wandb.log(wandb_dict, step=int(step))
      return summary
...
class BaseTrainer(abc.ABC):
  ...
  # Used for logging when multiple-dispatch function are compiled
    self._compiled_static_args = set()
    ...
    self.train_metrics_manager = MetricsManager(
        "train", summary_dir=summary_dir, log_to_wandb=use_wandb)

    # if self._log_every_n:
    #   # Training metric collected every `_log_every_n` steps
    #   self.fine_grained_train_mm = MetricsManager(
    #     f"train-freq", summary_dir=summary_dir, log_to_wandb=use_wandb)

    #   # Hack `self.fine_grained_train_mm` so it does not write to the log, just tensorboard/wandb
    #   _writers = self.fine_grained_train_mm._writer._writers
    #   _writers = tuple(x for x in _writers if not isinstance(x._writer, LoggingWriter))
    #   assert len(_writers) <= 1
    #   self.fine_grained_train_mm._writer._writers = _writers
    # else:
    #   self.fine_grained_train_mm = None
    ...
    self.eval_metrics_managers = {  # pylint:disable=g-complex-comprehension
        n: MetricsManager(f"training_eval/{n}", summary_dir=summary_dir, log_to_wandb=use_wandb)
        for n in eval_names
    }
  ...
  def _weight_metric_fn(self):
    """Compiled function to computes weight RMS and max"""
    def _get_weight_metrics_fn(_params):
      out = {
        f"weight_rms/{k}": metrics_lib.CurrentOnStep.from_model_output(
          jnp.sqrt(jnp.mean(jnp.square(v))))
        for k, v in utils.flatten_dict_string_keys(_params).items()
      }
      out.update({
        f"weight_max/{k}": metrics_lib.CurrentOnStep.from_model_output(
          jnp.max(jnp.abs(v)))
        for k, v in utils.flatten_dict_string_keys(_params).items()
      })
      return out

    _get_weight_metrics_fn_part = self._partitioner.partition(
      _get_weight_metrics_fn,
      (self._train_state_axes.params,),
      None
    )
    return _get_weight_metrics_fn_part
  ...
  def train(self,
    ...
    # Used for host coordination if using bucketed packing
    ...
    with self._train_state_mutex:
      ...
      for step_num in range(start_step, start_step + num_steps):
        logging.log_every_n_seconds(logging.INFO, "Training: step %d", 10, step_num)
    ...
    if metrics is not None:
      ...
      if self._log_weights:
        metrics.update(self._weight_metric_fn(train_state.params))
  ...
  def compile_train(self, batch: ElementSpec) -> None:
    ...
    Args:
      batch_spec: batch specfication from a dataset
    """
    tick = time.time()
    _compiled = self._partitioner.compile(
      self._partitioned_train_step, self.train_state, batch)
    self._compiled_train_step = _compiled
  ...
  def eval(
    ...
    for iter_name, batch_iter in batch_iters.items():
      ...
      if pbar_nsteps:
        pbar = tqdm(total=pbar_nsteps, ncols=100, desc=iter_name)
      else:
        pbar = None
...
def accumulate_grads_microbatched(
  ...
  Args:
    ...
    data_partition_spec: the PartitionSpec to use for partitioning annotations
      on the batch.
    loss_fn_args: Arguments to pass to the model's loss function
  ...
  if loss_fn_args:
    grad_fn = functools.partial(grad_fn, **loss_fn_args)
...
def apply_grads(
  ...
  metrics["learning_rate_avg"] = clu.metrics.Average.from_model_output(
      jnp.asarray([learning_rate]))
  metrics["learning_rate_current"] = clu.metrics.LastValue.from_model_output(
      jnp.asarray([learning_rate]))
...
def train_with_lr(
  ...
  grad_accum, metrics, flax_mutables = (
      accumulate_grads_microbatched(model, train_state, batch, dropout_rng,
                                    num_microbatches, data_partition_spec, loss_fn_args))
  ...
  def _partitioned_train_step(self) -> PartitionedTrainCallable:
    ...
    def train_step(train_state: train_state_lib.TrainState, batch: BatchType, static_args=None):
      ...
      if static_args is not None:
        # Call Transformer with these args set to be static
        assert self.packing_strategy and self.packing_strategy.use_static_packing_args
        loss_fn_args = dict(module_args=dict(static_args))
        loss_fn_args["module_args"]["use_constraints"] = True
        loss_fn_args.update(static_args)
      else:
        loss_fn_args["module_args"] = dict(use_constraints=True)

      if self.packing_strategy.pack_max_len:
          # Tells Transformer to pack examples
          max_lens = self.packing_strategy.pack_max_len
          loss_fn_args["module_args"].update(dict(
            fold=self.packing_strategy.max_to_pack,
            horizontally_pack_inputs=max_lens[0],
            horizontally_pack_targets=max_lens[1]
          ))

      state, metrics = train_with_lr(
          train_state,
          batch,
      ...
      if self._num_microbatches is not None and self._num_microbatches > 1:
        for k, v in metrics.items():
          if isinstance(v, AveragePerStep):
            logging.info(f"Dividng metric {k} for grad accumulation")
            metrics[k] = v.replace(total=v.total / self._num_microbatches)
