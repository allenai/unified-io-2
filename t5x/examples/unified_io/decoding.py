"""Decoding functions for UIO2, modified from t5x.decoding"""
import functools
...
def temperature_sample(
  ...
  log_prob = jnp.take_along_axis(log_prob, jnp.squeeze(idxs, axis=-1), axis=-1)
  all_logprob = jnp.take_along_axis(all_logprob, idxs, axis=1)
  # returns [batch, num_decodes, len], [batch, num_decodes] in sorted order.
  return decodes, log_prob, all_logprob
...
def _temperature_sample_single_trial(
    ...
    initial_index: Optional[jnp.ndarray] = None,
    max_decode_steps: Optional[int] = None,
    next_token_callback=None) -> jnp.ndarray:
  ...
  def sampling_loop_body_fn(state: SamplingLoopState) -> SamplingLoopState:
    ...
    if next_token_callback:
      next_token = next_token_callback(logits, next_token)
...
def beam_search(inputs: jnp.ndarray,
                ...
                decode_rng: Optional[jnp.ndarray] = None,
                cache_offset: int = 0,
                repetition_penalty: float = None) -> Tuple[jnp.ndarray, jnp.ndarray]:
  ...
  def beam_search_loop_body_fn(state: BeamState) -> BeamState:
    ...
    if repetition_penalty is not None:
      shift = jnp.reshape(jnp.repeat(jnp.arange(flat_ids.shape[0]) * flat_logits.shape[1], flat_ids.shape[1]), flat_ids.shape)
      penalty_logits = jnp.take(flat_logits, flat_ids + shift)
      penalty_logits = jnp.where(penalty_logits > 0, penalty_logits / repetition_penalty, penalty_logits * repetition_penalty)
      flat_logits = flat_logits.at[(jnp.repeat(jnp.arange(penalty_logits.shape[0]), penalty_logits.shape[1]), flat_ids.flatten())].set(penalty_logits.flatten())
