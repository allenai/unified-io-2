...
class DatasetConfig:
  ...
  target_field_name: str = 'text_targets'
...
# Excerpt from Big Vision codebase
# reference: https://github.com/google-research/big_vision
@gin.configurable
def create_vision_learning_rate_scheduler(
    ...
    constant_warmup = kw.get("constant_warmup", False)
    ...
    def step_fn(step):
        ...
        if warmup_steps and not constant_warmup:
  ...
  def infer_fn(ds: tf.data.Dataset,
    ...
    for index, infer_batch in sharded_ds.as_numpy_iterator():
      ...
      try:
        ...
        pass
      batch_result = jax.tree_map(lambda x: np.array(x), batch_result)
      ...
      n_done += 1
      if n_done % 20 == 0:
        logging.info(f"Completed batch {n_done} of {total}")

    logging.info(f'Inference of all {len(batched_results)} batches done.')
    all_inferences = batched_results

    # List[B * shard_count, ...] -> [B * shard_count * batch_count, ...]
    logging.info("Building numpy results...")
    ...
    indices_and_outputs = jax.tree_map(lambda x: np.array(x),
                                       indices_and_outputs)
    ...
    if aux_values is None:
      ...
      aux_values = jax.tree_map(lambda x: np.array(x), aux_values)
...
def get_dataset_inner(cfg: DatasetConfig,
  ...
  ds = mixture_or_task.get_dataset(
      ...
      # feature_converter  # pytype: disable=not-instantiable
      shard_info=shard_info,
      use_cached=cfg.use_cached,
      seed=seed,
      trim_output_features=cfg.trim_output_features)

  converter = feature_converter_cls(
        pack=cfg.pack, use_custom_packing_ops=cfg.use_custom_packing_ops)
  ds = converter(ds, cfg.task_feature_lengths)

  if batching_fn:
    ds = batching_fn(ds, batch_size=batch_size, drop_remainder=True)
  else:
