...
import dataclasses
from typing import MutableMapping, Optional, Union, Any
...
class CurrentOnStep(clu_metrics.Metric):
  """Represents current value on step.
  ...
  def from_model_output(cls,
                        values: Any,
                        **_) -> clu_metrics.Metric:
    """Initializes an CurrentOnStep Metric from array (or singular) values.
    ...
    Returns:
      CurrentOnStep object.
    """
    values = jnp.asarray(values)
    return cls(total=values)
  ...
  def compute(self) -> Scalar:
    if self.total is None:
      raise ValueError("`CurrentOnStep` `total` cannot be None.")
    return self.total
