...
def default_mesh(num_partitions: int,
  ...
  # seems device_kind can not reflect current tpu type.
  # set mannuly for now.

  if device_kind == 'Unknown TPU version':
    device_kind = 'TPU v4'
  ...
  def get_mesh_axes(self, train_state: TrainState) -> TrainState:
    ...
    def _logical_to_mesh_axes(param_name, logical_axes):
      ...
      try:
        ...
        raise ValueError(f'Failed to map logical axes for {param_name} axis={logical_axes}') from e
