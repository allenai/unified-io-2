...
def standard_logical_factor_rules():
  return freeze({
      ...
      'image_patch': FactorDim.NONE,
      'audio_patch': FactorDim.NONE,
      'embed': FactorDim.ROW,
      ...
      'axis_0': FactorDim.NONE,
      'axis_1': FactorDim.NONE,
      'axis_2': FactorDim.NONE,
      'axis_3': FactorDim.NONE,
      ...
      'modality_embedding': FactorDim.NONE,
  })
...
class Adafactor(OptimizerDef):
  ...
  def __init__(self,
    ...
    Args:
      ...
      beta2_cap: clipping threshold for beta2
    ...
    hyper_params = _AdafactorHyperParams(
        learning_rate, factored, multiply_by_parameter_scale, beta1, decay_rate, beta2_cap,
  ...
  @staticmethod
  def _decay_rate_pow(i: int, exponent: float = 0.8, threshold=None) -> float:
    """Default Adafactor second-moment decay schedule."""
    t = jnp.array(i, jnp.float32) + 1.0
    if threshold is not None:
      return jnp.minimum(threshold, 1.0 - t**(-exponent))
    else:
  ...
  def apply_param_gradient(self, step, hyper_params, param, state, grad, path):
    ...
    beta2_cap = hyper_params.beta2_cap
    ...
    elif weight_decay_rate:
      logging.info(f"weight decay of {weight_decay_rate} is "
                   f"not being multiplying by the learning rate")
    ...
    decay_rate = self._decay_rate_pow(step - step_offset, exponent=decay_rate, threshold=beta2_cap)
    ...
    skip_weight_decay = False
    if weight_decay_rate is not None:
      skip_weight_decay = (
        (path.endswith("/scale") and len(param.shape) == 1) or
        (path.endswith("/bias") and len(param.shape) == 1) or
        path.endswith("/embedding") or
        ("/discrete_vae/" in path) or ("/vit_image_encoder/" in path)
      )
      # if skip_weight_decay:
      #   logging.info(f"Skipping weight decay for {path}")

    if weight_decay_rate is not None and (not skip_weight_decay):
