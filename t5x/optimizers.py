...
class MultiOptimizer(OptimizerDef):
  ...
  def init_state(self, params):
    ...
    param_states_mu = jax.tree_map(lambda x: _Marker(), params)
    param_states_nu = jax.tree_map(lambda x: _Marker(), params)
    ...
    for focus, opt_def in zip(self.traversals, self.sub_optimizers):
      ...
      param_states_mu = _update_subtree_of_traversal(focus, param_states_mu,
                                                  ss.param_states[0].mu)

      param_states_nu = _update_subtree_of_traversal(focus, param_states_nu,
                                                  ss.param_states[0].nu)

    # Update state to None when param is not optimized by any sub optimizer.
    param_states_mu = jax.tree_map(
        lambda x: (None if isinstance(x, _Marker) else x), param_states_mu)

    param_states_nu = jax.tree_map(
        lambda x: (None if isinstance(x, _Marker) else x), param_states_nu)
  ...
  def apply_gradient(self, hyper_params, params, state, grads):
    ...
    new_param_states_mu = jax.tree_map(lambda x: _Marker(), params)
    new_param_states_nu = jax.tree_map(lambda x: _Marker(), params)
    for focus, opt_def, hp in it:
      ps = _subtree_from_traversal(focus, params)
      gs = _subtree_from_traversal(focus, grads)

      ss_mu = _subtree_from_traversal(focus, state.param_states[0].mu)
      ss_nu = _subtree_from_traversal(focus, state.param_states[0].nu)
      
      count = state.param_states[0].count
      prev_ss = OptimizerState(
          state.step, 
          param_states=(
              optax.ScaleByAdamState(
                  count=count,
                  mu=ss_mu,
                  nu=ss_nu),
              state.param_states[1],
              state.param_states[2],
          ))

      new_ps, new_ss = opt_def.apply_gradient(hp, ps, prev_ss, gs)
      
      new_params = _update_subtree_of_traversal(focus, new_params, new_ps)
      new_param_states_mu = _update_subtree_of_traversal(focus, new_param_states_mu,
                                                      new_ss.param_states[0].mu)
      new_param_states_nu = _update_subtree_of_traversal(focus, new_param_states_nu,
                                                      new_ss.param_states[0].nu)

    # Update state to None when param is not optimized by any sub optimizer.
    new_param_states_mu = jax.tree_map(
        lambda x: (None if isinstance(x, _Marker) else x), new_param_states_mu)

    new_param_states_nu = jax.tree_map(
        lambda x: (None if isinstance(x, _Marker) else x), new_param_states_nu)
    ...
    new_param_states = (
        optax.ScaleByAdamState(
            count=count,
            mu=new_param_states_mu,
            nu=new_param_states_nu),
        state.param_states[1],
        state.param_states[2])
  ...
  def derive_logical_axes(self, optimizer, param_logical_axes):
    ...
    param_states_mu = jax.tree_map(lambda x: _Marker(),
                                optimizer.state.param_states[0].mu)
    param_states_nu = jax.tree_map(lambda x: _Marker(),
                                optimizer.state.param_states[0].nu)
    for focus, opt_def in zip(self.traversals, self.sub_optimizers):
      if hasattr(opt_def, 'derive_logical_axes'):
        ps = _subtree_from_traversal(focus, param_logical_axes)

        ss_mu = _subtree_from_traversal(focus, optimizer.state.param_states[0].mu)
        ss_nu = _subtree_from_traversal(focus, optimizer.state.param_states[0].nu)
        ...
        param_states_mu = _update_subtree_of_traversal(focus, param_states_mu,
                                                    new_opt.state.param_states[0].mu)
        param_states_nu = _update_subtree_of_traversal(focus, param_states_nu,
                                                    new_opt.state.param_states[0].nu)
